...
for {
  ...
  Some(Right(pr1Id)) <- robotsRepo.publish(r1.id, board1.id, Duration.standardHours(3))
  r2 <- robotsRepo.create(user.id, "r2", Lang.Python)
  Some(Right(pr2Id)) <- robotsRepo.publish(r2.id, board1.id, Duration.standardHours(3))
  r3 <- robotsRepo.create(user.id, "r3", Lang.Python)
  Some(Right(pr3Id)) <- robotsRepo.publish(r3.id, board1.id, Duration.standardHours(3))
  r4 <- robotsRepo.create(user.id, "r4", Lang.Python)
  Some(Right(pr4Id)) <- robotsRepo.publish(r4.id, board1.id, Duration.standardHours(3))
