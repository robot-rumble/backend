...
update msg model =
    case msg of
                    ...
                    ( { model
                        | renderState =
                            case model.renderState of
                                Render renderState ->
                                    Render { renderState | turns = Array.push turnState.state renderState.turns }

                                _ ->
                                    Render { turns = Array.fromList [ turnState.state ], current_turn_num = 0 }
                      }
                    , Cmd.none
                    )
        ...
        GotRenderMsg renderMsg ->
            case model.renderState of
                Render state ->
                    ( { model | renderState = Render <| updateRender renderMsg state }, Cmd.none )
...
updateRender msg model =
    case msg of
        ChangeTurn dir ->
            { model
                | current_turn_num =
                    model.current_turn_num
                        + (case dir of
                            Next ->
                                if model.current_turn_num == Array.length model.turns - 1 then
                                    0
                            ...
                            Previous ->
                                if model.current_turn_num == 0 then
                                    0
...
isLoading model =
    case model.renderState of
        Render render ->
            Array.length render.turns /= model.totalTurns
    ...
    div [ class "game-bar", class "mb-3" ]
        ...
        , div [ class "d-flex justify-content-between" ]
            ...
            , case model.renderState of
                ...
                _ ->
                    p [ class "disappearing" ] [ text "Saved." ]
            ]
        ]
    ...
    case model.renderState of
        Render state ->
            case Array.get state.current_turn_num state.turns of
                Just turn ->
                    gameRenderer (gameObjs turn)

                Nothing ->
                    div [] [ text "Invalid turn." ]
